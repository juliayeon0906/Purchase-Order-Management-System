<!DOCTYPE html>
<html lang="en">
<?php
include 'db-connect.php';
require 'functions.php';
?>
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@4.4.1/dist/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
    <title>Purchase Order Search</title>
</head>
<body class="bg-body">
    <?php
    $poNo = $_GET["searchInput"];
    ?>
    <div class="container">
        <h1 class="m-2">Search Purchase Order</h1>
        
        <div class="border border-primary border-2 p-3 m-2">
            <h2>Purchase Order Information</h2>
            <?php
                if (!empty($poNo)) {
                    $listPOInfo = new functions();
                    $listPOInfo->listPOinfo274($poNo);
                } else {
                    echo "<p>Please enter a Purchase Order number to search.</p>";
                }
            ?>
        </div>
        <button class="backBtn btn btn-primary btn-sm m-2" onclick="location.href='index.php'">Back</button>
    </div>
</body>
</html>